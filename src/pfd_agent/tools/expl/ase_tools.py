...
from ...utils.common import generate_work_path
...
def optimize_structure(
    ...
    **kwargs
    ...
    Args:
        input_structure (str): Path to the input structure file (e.g., CIF, POSCAR).
        model_path (str): Path to the trained Deep Potential model directory.
    ...
    Returns:
        dict: A dictionary containing optimization results:
            - optimized_structure (str): Path to the final optimized structure file.
            - optimization_traj (str): Path to the optimization trajectory file, if available.
    ...
    try:
        ...
        return {
            "optimized_structure": str(output_file),
            "optimization_traj": str(traj_file),
...
def run_molecular_dynamics(
    ...
    **kwargs
    ...
    Args:
        initial_structure (str): Input atomic structure file (supports .xyz, .cif, etc.)
    ...
    Returns: A dictionary containing:
            - trajectory_list (List[str]): The paths of output trajectory files generated.
            - log_file (str): Path to the log file containing simulation output.
    ...
    # Convert str to Path for internal processing
    initial_path = Path(initial_structure)

    # Read initial structure
    atoms_ls = read(str(initial_path), index=':')
    ...
    result = {
        "trajectory_list": [str(p) for p in traj_list],
        "log_file": str(log_file),
        "message": f"MD simulation completed with {len(traj_list)} trajectory files"
    }

    return result


def get_base_model_path(model_path: Optional[str] = None) -> Optional[str]:
    ...
    Returns: The resolved model path as a string, or None if nothing can be determined.
    """
    ...
    # For unknown suffixes or non-existing paths: return as string
    return str(resolved)